...
export function OnboardingProvider({ children }: { children: React.ReactNode }) {
    ...
    const { data: onboardingData, isLoading: queryLoading } = useQuery({
        ...
        queryFn: () => apiRequest<{ success: boolean, data: { status: OnboardingStatus, version: number } }>(`${import.meta.env.VITE_API_BASE_URL}/api/v1/onboarding/status`),
    ...
    const completeMutation = useMutation({
        mutationFn: () => apiRequest<{ success: boolean }>(`${import.meta.env.VITE_API_BASE_URL}/api/v1/onboarding/complete`, {
            method: 'PATCH',
    ...
    const skipMutation = useMutation({
        mutationFn: () => apiRequest<{ success: boolean }>(`${import.meta.env.VITE_API_BASE_URL}/api/v1/onboarding/skip`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
        }),
        onSuccess: () => {
    ...
    const trackStepMutation = useMutation({
        mutationFn: (step: number) => apiRequest(`${import.meta.env.VITE_API_BASE_URL}/api/v1/onboarding/progress`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
