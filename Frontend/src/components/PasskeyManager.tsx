...
export function PasskeyManager({ onPasskeyChange }: PasskeyManagerProps) {
  ...
  const { data: passkeys = [], isLoading: loading } = useQuery({
    queryKey: ['passkeys'],
    queryFn: () =>
      apiRequest<{ success: boolean; data: Passkey[] }>(`${apiUrl}/api/v1/passkeys`).then(
        (res) => res.data,
      ),
    staleTime: 60000,
  });
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center gap-3 p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg text-yellow-600 dark:text-yellow-500">
            ...
            <p className="text-sm">
              Passkeys are not supported in this browser. Please use a modern browser like Chrome,
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="mt-1">
              Sign in securely without a password using Face ID, Touch ID, Windows Hello, or a
              security key.
            </CardDescription>
          </div>
                    ...
                    {renamingId === passkey.id ? (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          autoFocus
                        />
                        <Button size="sm" variant="ghost" onClick={() => handleRename(passkey.id)}>
                          Save
                        </Button>
                        <Button
                          size="sm"
                ...
                {renamingId !== passkey.id && (
                  <div className="relative">
                    <Button
                      ...
                      className="h-8 w-8"
                      onClick={() =>
                        setShowDropdown(showDropdown === passkey.id ? null : passkey.id)
                      }
                    >
