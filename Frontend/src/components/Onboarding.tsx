...
export default function Onboarding() {
    ...
    return (
        <div
            className={`fixed inset-0 z-50 flex items-center justify-center ${isDark ? 'bg-black/80' : 'bg-gray-900/20'
                } backdrop-blur-md`}
            role="dialog"
            aria-modal="true"
        ...
        >
            <div
                className={`relative w-full max-w-2xl mx-4 rounded-2xl shadow-2xl ${isDark ? 'bg-neutral-900 border border-neutral-800' : 'bg-white'
                    }`}
            >
                {/* Skip button - always visible, one click */}
                <button
                    onClick={handleSkip}
                    className={`absolute top-4 right-4 p-2 rounded-lg transition-colors ${isDark ? 'hover:bg-neutral-800 text-neutral-400' : 'hover:bg-gray-100 text-gray-600'
                        }`}
                ...
                <div className="px-8 pt-8 pb-4">
                    <div className="flex gap-2">
                        {Array.from({ length: TOTAL_STEPS }).map((_, i) => (
                            <div
                                key={i}
                                className={`h-1 flex-1 rounded-full transition-colors duration-200 ${i < currentStep
                                    ? 'bg-[#DDEF00]'
                                    : isDark
                                        ? 'bg-neutral-800'
                                        : 'bg-gray-200'
                ...
                {/* Navigation */}
                <div className={`px-8 py-6 flex items-center justify-between border-t ${isDark ? 'border-neutral-800' : 'border-gray-200'}`}>
                    <button
                        onClick={handlePrevious}
                        disabled={currentStep === 1}
                        className={`px-4 py-2 rounded-lg font-medium transition-all ${currentStep === 1
...
function WelcomeStep({ user, isDark }: { user: any; isDark: boolean }) {
    return (
        <div className="space-y-6 py-8">
            ...
            <h1 id="onboarding-title" className={`text-4xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
        ...
        <div className="space-y-6 py-8">
            ...
            <div className="grid gap-4">
                {features.map((feature, i) => (
                    <div
                        key={i}
                        className={`p-4 rounded-xl border transition-all hover:scale-[1.02] ${isDark ? 'bg-neutral-800/50 border-neutral-700 hover:border-[#DDEF00]/50' : 'bg-gray-50 border-gray-200 hover:border-[#DDEF00]'
                            }`}
                    >
    ...
    return (
        <div className="space-y-6 py-8">
            ...
            <div className="space-y-4">
                <div className={`p-4 rounded-xl ${isDark ? 'bg-[#DDEF00]/10 border border-[#DDEF00]/20' : 'bg-[#DDEF00]/10 border border-[#DDEF00]/30'}`}>
                    ...
                    <div className="flex gap-2">
                        {['light', 'dark', 'system'].map((t) => (
                            <button
                                ...
                                className={`flex-1 px-4 py-2.5 rounded-lg font-medium transition-all ${theme === t
...
function LaunchStep({ user, isDark }: { user: any; isDark: boolean }) {
    return (
        <div className="space-y-6 py-8 text-center">
            ...
            <p className={`text-lg ${isDark ? 'text-neutral-400' : 'text-gray-600'}`}>
                You're ready to start using FairArena, {user?.firstName}. Let's build something amazing together!
            </p>
            <div className={`inline-block p-4 rounded-xl ${isDark ? 'bg-[#DDEF00]/10' : 'bg-[#DDEF00]/20'}`}>
