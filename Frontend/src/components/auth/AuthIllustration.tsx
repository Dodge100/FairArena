...
export const AuthIllustration: React.FC<AuthIllustrationProps> = ({ title, subtitle, icon, features }) => {
                                ...
                                <div className="flex flex-col gap-10">
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        >
                                            {title.split(' ').map((word, i) => (
                                                <span key={i} className={word.toLowerCase() === 'fairarena' ? 'text-[#DDEF00]' : ''}>
                                                    {word}{' '}
                                                </span>
                                            ))}
                                        </motion.h2>
                                        ...
                                        {features.map((item, i) => (
                                            ...
                                            >
                                                ...
                                                <div className="space-y-0.5">
                                                    ...
                                                    <div className="text-[10px] text-neutral-500 uppercase tracking-widest font-black leading-none line-clamp-1">{item.desc}</div>
                                                </div>
                                            </motion.div>
                                        ))}
                                    </div>
                                    ...
                                    >
                                        <div className="flex -space-x-3">
                                            {[1, 2, 3, 4].map((i) => (
                                                <div key={i} className={`w-10 h-10 rounded-full border-2 border-[#0d0d0d] bg-neutral-800 flex items-center justify-center overflow-hidden`}>
                                                    <div className={`w-full h-full bg-gradient-to-br ${i === 1 ? 'from-yellow-400' : i === 2 ? 'from-[#DDEF00]' : i === 3 ? 'from-white' : 'from-neutral-400'} opacity-20`} />
                                                </div>
                                            ))}
                                        </div>
                                    </motion.div>
