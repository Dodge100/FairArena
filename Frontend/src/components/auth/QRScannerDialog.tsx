  ...
  const approveMutation = useMutation({
    mutationFn: (sessionId: string) =>
      apiRequest<{ success: boolean; message?: string }>(
        `${import.meta.env.VITE_API_BASE_URL}/api/v1/auth/qr/approve`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ sessionId }),
        ...
        <div className="p-6">
          {step === 'scan' ? (
            <div className="space-y-4">
              <QRScanner onScan={handleScan} onError={(err) => console.log('Scanner:', err)} />
              {loading && (
            ...
            <div className="space-y-5">
                ...
                <div className="flex items-start gap-4">
                  <div
                    className={`w-14 h-14 rounded-xl flex items-center justify-center shrink-0 ${
                      isDark ? 'bg-neutral-700' : 'bg-white shadow-sm'
                    }`}
                  >
                    {getDeviceIcon()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3
                      className={`font-semibold text-base truncate ${isDark ? 'text-white' : 'text-neutral-900'}`}
                    >
                      {approvalReq?.deviceInfo.browser} on {approvalReq?.deviceInfo.os}
                    </h3>
                    <p
                      className={`text-sm mt-0.5 ${isDark ? 'text-neutral-400' : 'text-neutral-500'}`}
                    >
                      {approvalReq?.deviceInfo.device} •{' '}
                      {approvalReq?.deviceInfo.location || 'Unknown location'}
                    </p>
              ...
              >
                ...
                <div>
                  <p
                    className={`text-sm font-medium ${isDark ? 'text-yellow-400' : 'text-yellow-700'}`}
                  >
                    Security check
                  </p>
                  <p
                    className={`text-sm mt-0.5 ${isDark ? 'text-yellow-400/70' : 'text-yellow-600'}`}
                  >
                    Only approve if <strong>you</strong> initiated this sign-in request on the other
                    device.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  disabled={loading}
                  className={`py-3.5 px-4 rounded-xl font-semibold transition-all active:scale-[0.98] disabled:opacity-50 ${
                    isDark
