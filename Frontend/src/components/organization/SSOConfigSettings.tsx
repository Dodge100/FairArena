...
export const SSOConfigSettings: React.FC<Props> = ({ organizationId }) => {
  ...
  const apiClient = {
    ...
    post: (url: string, body: any) =>
      apiRequest<any>(`${baseUrl}/api/v1${url}`, {
        method: 'POST',
  ...
  const loadVerificationStatus = async () => {
    try {
      const res = await apiClient.get(
        `/organization/${organizationId}/sso-config/verification-status`,
      );
      if (res.data.success) {
  ...
  const handleInitiateVerification = async () => {
    try {
      const res = await apiClient.post(
        `/organization/${organizationId}/sso-config/verify-domain/initiate`,
        {},
      );
      if (res.data.success) {
  ...
  const handleVerifyDomain = async () => {
    ...
    try {
      const res = await apiClient.post(
        `/organization/${organizationId}/sso-config/verify-domain/check`,
        {},
      );
      if (res.data.success) {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure OIDC-based SSO for your organization. Users with matching email domains will
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, authorizationUrl: e.target.value })
                      }
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-neutral-500">Activate SSO for this domain</p>
                  </div>
                  <Switch
                ...
                <div className="flex gap-3">
                  <Button
                    type="submit"
                    disabled={saving}
                    className="bg-[#DDEF00] text-black hover:bg-[#cbe600]"
                  >
