...
import { Calendar, ExternalLink, Filter, Globe, Info, MapPin, Search, Trophy, Users, X } from 'lucide-react';
...
const Hackathons = () => {
    ...
    const fetchHackathons = async () => {
        try {
            setLoading(true);
            const response = await apiRequest<{ success: boolean; data: HackathonDetail[] }>(`${import.meta.env.VITE_API_BASE_URL}/api/v1/hackathons`);
            if (response.success) {
    ...
    const filteredHackathons = useMemo(() => {
        return hackathons.filter((h) => {
            ...
            const matchesSearch = (
                (h.title?.toLowerCase().includes(searchQuery.toLowerCase()) || false) ||
                (h.theme?.toLowerCase().includes(searchQuery.toLowerCase()) || false) ||
                (h.description?.toLowerCase().includes(searchQuery.toLowerCase()) || false)
            );
            ...
            const matchesPrice = selectedPrice === 'all' || (() => {
                ...
                if (selectedPrice === 'paid') return !fee.includes('free') && !fee.includes('0') && fee.length > 0;
                return true;
            })();
    ...
    const hasActiveFilters = searchQuery || selectedTheme !== 'all' || selectedStatus !== 'all' || selectedPrice !== 'all';
                ...
                <Card className="group flex flex-col h-full hover:shadow-md transition-all duration-200 cursor-pointer border-border/60 hover:border-border/80 bg-card overflow-hidden">
                    <CardHeader className="pb-3 space-y-3">
                        ...
                        <div className="flex flex-wrap items-start gap-2 mb-1 w-full overflow-hidden">
                            {validTheme && (
                                <Badge variant="secondary" className="font-normal rounded-md px-2.5 py-0.5 whitespace-nowrap text-left max-w-[140px] truncate">
                                    {validTheme}
                                </Badge>
                            )}
                            {hackathon.registrationFee && (
                                <Badge variant="outline" className={cn(
                                    ...
                                    (hackathon.registrationFee.toLowerCase().includes('free') || hackathon.registrationFee.includes('0'))
                        ...
                        <div className="space-y-1">
                            <h3 className="font-semibold text-lg leading-tight group-hover:text-primary transition-colors line-clamp-2 min-h-[1.5em] tracking-tight" title={hackathon.title}>
                                {hackathon.title}
                            </h3>
                            <div className="flex items-center gap-1.5 text-xs text-muted-foreground w-full">
                                <MapPin className="h-3.5 w-3.5 shrink-0" />
                                <span className="truncate w-full">{hackathon.aboutOrganizer || 'Organizer not specified'}</span>
                            </div>
                        </div>
                    </CardHeader>
                    ...
                    <CardFooter className="pt-3 border-t bg-muted/5 p-4 flex justify-between items-center text-xs text-muted-foreground mt-auto w-full">
                        <div className="flex items-center gap-1.5 shrink-0">
                            ...
                            <span className="truncate max-w-[100px]">{hackathon.registrationDeadline || 'TBA'}</span>
                        </div>
                        <div className="flex items-center gap-1.5 shrink-0">
                            <Trophy className="h-3.5 w-3.5 shrink-0" />
                            <span className="font-medium truncate max-w-[80px] text-right">{hackathon.totalPrizePool || 'Prizes'}</span>
                        </div>
                    </CardFooter>
                </Card>
            </DialogTrigger>
                ...
                <div className="flex-none p-6 pb-4 border-b bg-card z-10">
                    <div className="space-y-1 mb-4">
                        <div className="flex flex-wrap items-center gap-2 mb-3">
                            {validTheme && (
                                <Badge variant="secondary" className="font-normal rounded-md px-2.5 py-1 whitespace-normal text-left leading-snug h-auto max-w-full">
                                    {validTheme}
                                </Badge>
                            )}
                            {hackathon.registrationFee && (
                                <Badge variant="outline" className="font-normal rounded-md border-dashed px-2.5 py-1 whitespace-normal text-left leading-snug h-auto max-w-full">
                    ...
                    <div className="flex flex-wrap gap-x-8 gap-y-4 pt-2">
                        <SimpleStat label="Deadline" value={hackathon.registrationDeadline} icon={<Calendar className="h-4 w-4" />} />
                        <SimpleStat label="Starts" value={hackathon.hackathonStarts} icon={<Calendar className="h-4 w-4" />} />
                        <SimpleStat label="Team Size" value={hackathon.teamComposition} icon={<Users className="h-4 w-4" />} />
                        <SimpleStat label="Prize Pool" value={hackathon.totalPrizePool} icon={<Trophy className="h-4 w-4" />} />
                    </div>
                </div>
                ...
                <div className="flex-grow overflow-y-auto bg-muted/5 p-6 space-y-8 scroll-smooth">
                    <section className="space-y-3">
                        ...
                        <div className="text-sm leading-7 text-foreground/90 whitespace-pre-wrap break-words">
                            {hackathon.overview || hackathon.description || 'No detailed overview available provided.'}
                        </div>
                    </section>
                        ...
                        {(hackathon.firstWinner || hackathon.firstRunnerUp || hackathon.secondRunnerUp) && (
                            <section className="space-y-3">
                                ...
                                <div className="space-y-2 rounded-lg border bg-card p-4">
                                    {hackathon.firstWinner && <PrizeRow place="1st Place" value={hackathon.firstWinner} />}
                                    {hackathon.firstRunnerUp && <PrizeRow place="2nd Place" value={hackathon.firstRunnerUp} />}
                                    {hackathon.secondRunnerUp && <PrizeRow place="3rd Place" value={hackathon.secondRunnerUp} />}
                                </div>
                            </section>
                        )}
                            ...
                            <div className="flex flex-wrap gap-4 text-sm">
                                {hackathon.contactEmail && (
                                    <div className="flex items-center gap-2 text-muted-foreground">
                                        <span className="font-medium text-foreground">Email:</span> <span className="break-all select-all">{hackathon.contactEmail}</span>
                                    </div>
                                )}
                                {hackathon.contactPhoneNo && (
                                    <div className="flex items-center gap-2 text-muted-foreground">
                                        <span className="font-medium text-foreground">Phone:</span> {hackathon.contactPhoneNo}
                                    </div>
                                )}
                            </div>
                        </section>
...
const SimpleStat = ({ label, value, icon }: { label: string; value?: string; icon: React.ReactNode }) => {
...
const PrizeRow = ({ place, value }: { place: string; value: string }) => (
    <div className="flex justify-between items-center text-sm border-b last:border-0 pb-2 last:pb-0 border-border/50">
        <span className="text-muted-foreground font-medium text-xs uppercase tracking-wide">{place}</span>
