  ...
  const handleVerify = async () => {
    ...
    try {
      const response = await apiClient.get(
        `/api/v1/oauth/device/verify?user_code=${userCode.trim()}`,
      );
      ...
      <div className="flex min-h-screen items-center justify-center bg-background p-4">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              You have denied access to this device. The application will not be able to access your
              account.
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <Card className="w-full max-w-md">
        <CardHeader>
          ...
          <CardTitle className="text-center">
            {deviceRequest.application?.name || 'Unknown Application'}
          </CardTitle>
          <CardDescription className="text-center">
        ...
        <CardContent className="space-y-4">
            ...
            <ul className="space-y-2">
              {(deviceRequest.scopes || []).map((scope) => (
                <li key={scope.name} className="flex items-start gap-2 text-sm">
                  <Shield
                    className={`mt-0.5 h-4 w-4 flex-shrink-0 ${scope.isDangerous ? 'text-destructive' : 'text-muted-foreground'}`}
                  />
                  <div>
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={() => handleConsent('approve')} disabled={loading} className="flex-1">
              {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Authorize
            </Button>
          </div>
