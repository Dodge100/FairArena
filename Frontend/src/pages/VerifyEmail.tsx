      ...
      case 'error':
        return (
          <>
            ...
            <div
              className={`mt-6 pt-6 border-t ${isDark ? 'border-neutral-800' : 'border-neutral-200'}`}
            >
              <h3
                className={`text-sm font-semibold mb-3 ${isDark ? 'text-white' : 'text-neutral-900'}`}
              >
            ...
            <Link
              to="/signin"
              className={`mt-6 inline-block text-sm hover:underline ${isDark ? 'text-neutral-400' : 'text-neutral-600'}`}
            >
              Back to Sign In
            </Link>
          </>
    ...
    >
      ...
      {showCaptchaModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div
            className={`relative w-full max-w-sm rounded-2xl p-6 shadow-2xl transform transition-all scale-100 ${isDark ? 'bg-neutral-900 border border-neutral-800' : 'bg-white'}`}
          >
            <button
            ...
            <h3
              className={`text-lg font-bold mb-2 text-center ${isDark ? 'text-white' : 'text-neutral-900'}`}
            >
              Security Check
            </h3>
            <p
