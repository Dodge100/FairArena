    ...
    <div className="flex min-h-screen w-full items-center justify-center bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-4">
      ...
      <Card className="relative z-10 w-full max-w-2xl border-slate-700 bg-slate-800/90 backdrop-blur-xl shadow-2xl">
        <CardHeader className="text-center pb-4">
          ...
          <CardTitle className="text-3xl font-bold text-white">{t('ipBlocked.title')}</CardTitle>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            ...
            >
              {t('ipBlocked.retry')}
              {countdown > 0 && <span className="ml-2 text-xs opacity-80">({countdown}s)</span>}
            </Button>
            <Button
          ...
          {countdown === 0 && (
            <div className="text-center">
              <p className="text-sm text-slate-400 animate-pulse">{t('ipBlocked.autoRetry')}</p>
            </div>
          )}
        </CardContent>
      </Card>
