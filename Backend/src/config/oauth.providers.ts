...
export const OAUTH_PROVIDERS: Record<string, OAuthProviderConfig> = {
  ...
  huggingface: {
    ...
    extractUserData: (hfUser: any): OAuthUserData => ({
      ...
      lastName:
        hfUser.name?.split(' ').slice(1).join(' ') ||
        hfUser.fullname?.split(' ').slice(1).join(' ') ||
        null,
  ...
  vercel: {
    ...
    callbackUrl: ENV.VERCEL_CALLBACK_URL,
    scopes: ['user'],
    responseType: 'code',
    grantType: 'authorization_code',
    extractUserData: (vercelUser: any): OAuthUserData => ({
      providerId: vercelUser.user?.id || vercelUser.id,
      email: vercelUser.user?.email || vercelUser.email,
      emailVerified: true,
      firstName: vercelUser.user?.name?.split(' ')[0] || vercelUser.name?.split(' ')[0] || null,
      lastName:
        vercelUser.user?.name?.split(' ').slice(1).join(' ') ||
        vercelUser.name?.split(' ').slice(1).join(' ') ||
        null,
      profileImageUrl: vercelUser.user?.avatar || vercelUser.avatar || null,
      providerName: 'vercel',
    }),
  },
  ...
  figma: {
    ...
    callbackUrl: ENV.FIGMA_CALLBACK_URL,
    scopes: ['file_read'],
    responseType: 'code',
    grantType: 'authorization_code',
    extractUserData: (figmaUser: any): OAuthUserData => ({
      providerId: figmaUser.id,
      email: figmaUser.email,
      emailVerified: true,
      firstName: figmaUser.handle || null,
      lastName: null,
      profileImageUrl: figmaUser.img_url || null,
      providerName: 'figma',
    }),
  },
  ...
  zoom: {
    ...
    callbackUrl: ENV.ZOOM_CALLBACK_URL,
    scopes: ['user:read'],
    responseType: 'code',
    grantType: 'authorization_code',
    extractUserData: (zoomUser: any): OAuthUserData => ({
      providerId: zoomUser.id,
      email: zoomUser.email,
      emailVerified: zoomUser.verified === 1,
      firstName: zoomUser.first_name || null,
      lastName: zoomUser.last_name || null,
      profileImageUrl: zoomUser.pic_url || null,
      providerName: 'zoom',
    }),
  },
  ...
  atlassian: {
    ...
    responseType: 'code',
    grantType: 'authorization_code',
    extractUserData: (atlassianUser: any): OAuthUserData => ({
      providerId: atlassianUser.account_id,
      email: atlassianUser.email,
      emailVerified: true,
      firstName: atlassianUser.name?.split(' ')[0] || null,
      lastName: atlassianUser.name?.split(' ').slice(1).join(' ') || null,
      profileImageUrl: atlassianUser.picture || null,
