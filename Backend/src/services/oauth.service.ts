...
import { getOAuthProvider, type OAuthProviderConfig, type OAuthUserData } from '../config/oauth.providers.js';
import {
...
export async function handleGenericOAuthCallback(
    ...
    try {
        ...
        if (!code || typeof code !== 'string') {
            return res.redirect(
                `${ENV.FRONTEND_URL}/signin?error=${providerName}_auth_failed`
            );
        }
        ...
        if (errorMessage === 'EMAIL_MISSING') {
            return res.redirect(
                `${ENV.FRONTEND_URL}/signin?error=${providerName}_email_missing`
            );
        }
