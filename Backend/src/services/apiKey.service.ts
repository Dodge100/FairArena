...
export async function updateApiKeyLastUsed(apiKeyId: string, ipAddress: string): Promise<void> {
    ...
    try {
        ...
        logger.warn('Failed to update API key last used', { apiKeyId, error: (error as Error).message });
    }
}
 ...
 * Update API key name
 */
export async function updateApiKeyName(userId: string, apiKeyId: string, name: string): Promise<boolean> {
    try {
        ...
        logger.error('Failed to update API key name', { userId, apiKeyId, error: (error as Error).message });
        throw error;
    }
}
