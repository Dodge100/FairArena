  ...
  try {
    ...
    if (keys.length === 0 && ENV.OAUTH_BOOTSTRAP_RSA_PUBLIC_KEY) {
      ...
      const keyObject = crypto.createPublicKey(
        ENV.OAUTH_BOOTSTRAP_RSA_PUBLIC_KEY.replace(/\\n/g, '\n'),
      );
...
export async function authorizeEndpoint(req: Request, res: Response): Promise<void> {
  ...
  const needsConsent =
    application.isTrusted === false && (newScopesNeeded.length > 0 || !existingConsent);
...
async function processConsent(
  ...
  const scopeLabels = scopes.map((scope) => {
    switch (scope) {
      ...
      case 'offline_access':
        return "Access your data when you're not using the app";
      default:
        return scope;
    }
  });
...
export async function getAuthorizationRequest(req: Request, res: Response): Promise<void> {
  ...
  const scopeDescriptions: Record<
    string,
    { displayName: string; description: string; isDangerous: boolean }
  > = {
    openid: { displayName: 'OpenID', description: 'Verify your identity', isDangerous: false },
    profile: {
      displayName: 'Profile',
      description: 'Access your profile information (name, picture)',
      isDangerous: false,
    },
...
async function handleAuthorizationCodeGrant(
  ...
  if (authCode.codeChallenge) {
    ...
    if (
      !verifyPkce(code_verifier, authCode.codeChallenge, authCode.codeChallengeMethod || 'S256')
    ) {
      res.status(400).json({
        error: 'invalid_grant',
...
async function handleDeviceCodeGrant(
  ...
  const { hashDeviceCode, getDeviceCodeByHash, updateDeviceCodePollTime } =
    await import('../../services/oauthProvider.service.js');
...
export async function deviceAuthorizeEndpoint(req: Request, res: Response): Promise<void> {
  ...
  // Generate codes
  const { generateDeviceCode, generateUniqueUserCode, storeDeviceCode } =
    await import('../../services/oauthProvider.service.js');
...
export async function getDeviceRequest(req: Request, res: Response): Promise<void> {
  ...
  const { getDeviceCodeHashByUserCode, getDeviceCodeByHash } =
    await import('../../services/oauthProvider.service.js');
...
export async function deviceConsentEndpoint(req: Request, res: Response): Promise<void> {
  ...
  const { getDeviceCodeHashByUserCode, getDeviceCodeByHash, updateDeviceCodeStatus } =
    await import('../../services/oauthProvider.service.js');
