...
import { createVerificationRecord, generateVerificationToken, verifyDomainOwnership } from '../../services/dnsVerification.service.js';
...
export const upsertSSOConfig = async (req: Request, res: Response) => {
    try {
        ...
        const FORBIDDEN_DOMAINS = [
            ...
            'googlemail.com', 'yandex.com', 'mail.ru', 'zohomail.in'
        ];

        if (FORBIDDEN_DOMAINS.includes(data.domain.toLowerCase())) {
            return res.status(400).json({
                success: false,
                message: 'Security Error: Cannot enable SSO for public email domains. Please use a custom corporate domain.'
            });
        }
        ...
        if (data.isActive) {
            ...
            if (!currentConfig?.domainVerified) {
                return res.status(403).json({
                    success: false,
                    message: 'Cannot activate SSO without domain verification. Please verify domain ownership first.'
                });
            }
        }
...
export const testSSOConnection = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!config || !config.issuerUrl) return res.status(400).json({ success: false, message: 'No config' });
        ...
        try {
            ...
            return res.status(400).json({ success: false, message: 'Could not connect to Provider. Check Issuer URL.' });
        }
    } catch (e) {
