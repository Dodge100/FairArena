...
export const intrusionDetection = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (isBlocked) {
      logSecurityEvent(SecurityEventType.IP_BLOCKED, 'Blocked IP attempted access', {
        ip,
        path: req.path,
        method: req.method,
      });
    ...
    if (INTRUSION_DETECTION_CONFIG.honeypotPaths.some((path) => req.path.startsWith(path))) {
      logSecurityEvent(SecurityEventType.HONEYPOT_TRIGGERED, 'Honeypot endpoint accessed', {
        ip,
        path: req.path,
        method: req.method,
    ...
    const scanValue = (value: any, type: string): boolean => {
      if (typeof value === 'string') {
        ...
        if (detected) {
          ...
          // Track violation
          const threshold =
            INTRUSION_DETECTION_CONFIG.thresholds[
 ...
 */
export const trackAuthFailures = async (req: Request, res: Response, next: NextFunction) => {
  const ip =
    (req.headers['x-forwarded-for'] as string)?.split(',')[0]?.trim() || req.ip || 'unknown';
