    ...
    try {
        ...
        try {
            ...
            if (cachedPermissions !== null) {
                const data =
                    typeof cachedPermissions === 'string'
                        ? JSON.parse(cachedPermissions)
                        : cachedPermissions;
                req.organizationContext = data;
                logger.debug('Loaded organization permissions from cache', { userId, slug });
                return next();
            }
        } catch (cacheError) {
        ...
        try {
            ...
            logger.debug('Cached organization permissions', { userId, slug, cacheKey });
        } catch (cacheError) {
...
export const requirePermission = (
    category: keyof OrganizationPermissions,
    action: string,
) => {
