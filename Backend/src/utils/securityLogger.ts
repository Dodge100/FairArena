...
export function logRateLimitViolation(meta: SecurityEventMeta) {
  logSecurityEvent(SecurityEventType.RATE_LIMIT_EXCEEDED, 'Rate limit exceeded', meta);
}

export function logCsrfViolation(meta: SecurityEventMeta) {
  logSecurityEvent(SecurityEventType.CSRF_TOKEN_INVALID, 'CSRF token validation failed', meta);
}
...
export function logSessionAnomaly(reason: string, meta: SecurityEventMeta) {
  logSecurityEvent(SecurityEventType.SESSION_ANOMALY, `Session anomaly detected: ${reason}`, meta);
}

export function logUnauthorizedAccess(meta: SecurityEventMeta) {
  logSecurityEvent(SecurityEventType.UNAUTHORIZED_ACCESS, 'Unauthorized access attempt', meta);
}
